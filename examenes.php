...
// SELECT MATCH (_usuarios_productos.titulo,_usuarios_productos.descripcion, _usuarios_productos.palabrasclaves) AGAINST ('*" . $busqueda . "*' IN NATURAL LANGUAGE MODE) as relevancia, _usuarios_productos.* from _usuarios_productos where _usuarios_productos.idestatusproducto = '1' and MATCH (_usuarios_productos.titulo,_usuarios_productos.descripcion, _usuarios_productos.palabrasclaves) AGAINST ('*" . $busqueda . "*' IN NATURAL LANGUAGE MODE)  and '".$busqueda."' like CONCAT('%', _usuarios_productos.palabrasclaves ,'%') order by 1 DESC limit 0,1
$sql_infousuario=mysqli_query($conexionbd,"SELECT * from examenes order by nombre ASC");
$cont = 0;
